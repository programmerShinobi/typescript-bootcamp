...
interface incomeInterface {
    GetTotalIncomeVechile(data?: string): object;
}


class purchase implements purchaseInterface{
    constructor(protected ArrayProp: any[]) {}
...
class income extends purchase implements incomeInterface{
    ...
    GetTotalIncomeVechile(data?: string | undefined): object {
        let count = 0;
        if (data) {
            for (const vehicle of this.ArrayProp) {
                if (vehicle.purch_vehicleType == data) {
...
console.info(vehicleIncome.GetTotalIncomeVechile(SUV));
console.info(vehicleIncome.GetTotalIncomeVechile(TAXI));
console.info(vehicleIncome.GetTotalIncomeVechile(PrivateJet));
console.info(vehicleIncome.GetTotalIncomeVechile());
console.info(vehicleIncome.GetData());